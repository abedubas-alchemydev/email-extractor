...
async def test_enrich_no_match_leaves_phone_null(patch_settings: None) -> None:
    ...
    respx.post(_APOLLO_MATCH_URL).mock(return_value=httpx.Response(200, json={}))
...
def patch_reveal_configured(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(settings, "public_base_url", "https://app.example.com/api/backend")
...
async def test_enrich_requests_personal_email_reveal(patch_settings: None) -> None:
    ...
    route = respx.post(_APOLLO_MATCH_URL).mock(return_value=httpx.Response(200, json={"person": {"name": "Jane Doe"}}))
...
async def test_enrich_requests_phone_reveal_when_configured(
    ...
    route = respx.post(_APOLLO_MATCH_URL).mock(
        return_value=httpx.Response(200, json={"person": {"id": "p1", "name": "Jane Doe"}})
    )
    ...
    assert sent["webhook_url"] == ("https://app.example.com/api/backend/api/v1/webhooks/apollo/sekret/phone-reveal")
...
async def test_enrich_falls_back_to_work_email_when_no_personal(patch_settings: None) -> None:
    ...
    respx.post(_APOLLO_MATCH_URL).mock(
        return_value=httpx.Response(200, json={"person": {"name": "Jane Doe", "email": "jane.doe@firm.com"}})
    )
