...
async def test_complete_match_stops_chain_early(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    full = EnrichmentData(name="J", title="T", company="C", linkedin_url="L", email="e@x.com", phone="p")
...
def _wire_reveal_configured(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(settings, "public_base_url", "https://app.example.com/api/backend", raising=False)
    ...
    respx.post(snov_mod.PROFILE_BY_EMAIL_URL).mock(
        return_value=httpx.Response(
            ...
            json={
                ...
                "firstName": "Liz",
                "lastName": "Hamer",
                "currentJobs": [{"position": "Regional Creative Director", "companyName": "Globex"}],
                "social": [{"link": "https://www.linkedin.com/in/lizihamer/", "type": "linkedIn"}],
            },
        )
    )
...
async def test_snov_no_profile_is_clean_miss(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    respx.post(snov_mod.PROFILE_BY_EMAIL_URL).mock(return_value=httpx.Response(200, json={"success": False}))
...
async def test_name_lookup_skips_when_no_fillable_field_needed(
    ...
    out = await name_lookup_mod.NameLookupEnrichProvider().enrich("jane.doe@acme.com", needed={"title", "company"})
        ...
        *, domain: str, org_name: str, people: list[Any], crawler: Any = None, linkedin: Any = None
        ...
        return {
            0: DiscoveryResult(
                ...
                raw={},
                emails=[],
                phones=[PhoneHit(value="(555) 123-4567", type="work", confidence=70.0, source="web_fallback")],
            )
        }
