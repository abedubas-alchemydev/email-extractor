...
def email_matches_person(local_part: str, first: str, last: str) -> bool:
    ...
    return len(fn) >= 6 and fn in parts
...
def _person_anchor_positions(text: str, first: str, last: str, emails: set[str]) -> list[int]:
    ...
    *,
    ...
    for person in people:
        ...
        for local, full in crawled_emails:
            ...
            email_hits.append(EmailHit(value=full, type="work", confidence=conf, source=_PROVIDER_NAME))
        ...
        if phones_enabled and page_texts:
            ...
            for text in page_texts.values():
                anchors = _person_anchor_positions(text, first.lower(), last.lower(), matched_emails)
                if not anchors:
                    continue
