...
class PdlProvider(ContactDiscoveryProvider):
    ...
    async def find_person(
        ...
        return await _request_and_parse(payload, api_key, log_context=f"{first_name} {last_name}")
...
async def _request_and_parse(
    ...
    *,
    ...
    if response.status_code != 200:
        logger.info("PDL person enrich returned %d for %s", response.status_code, log_context)
        return None
