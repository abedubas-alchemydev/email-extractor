...
class ApolloMatchProvider(ContactDiscoveryProvider):
    ...
    async def find_person(
        ...
        confidence = _EMAIL_STATUS_CONFIDENCE.get(email_status, 30.0) if email else 0.0
        ...
        apollo_person_id = str(person_id_raw).strip()[:64] if isinstance(person_id_raw, (str, int)) else None
        ...
        if (
            ...
            fb_linkedin = (fallback.get("linkedin_url") or None) if isinstance(fallback, dict) else None
