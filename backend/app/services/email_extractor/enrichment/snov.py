...
async def _post_profile(email: str, token: str, client_id: str, client_secret: str) -> httpx.Response | None:
    ...
    try:
        async with httpx.AsyncClient(timeout=settings.snov_request_timeout) as client:
            response = await client.post(PROFILE_BY_EMAIL_URL, json=body, headers={"Authorization": f"Bearer {token}"})
    ...
    try:
        async with httpx.AsyncClient(timeout=settings.snov_request_timeout) as client:
            return await client.post(PROFILE_BY_EMAIL_URL, json=body, headers={"Authorization": f"Bearer {new_token}"})
    except httpx.HTTPError as exc:
