...
class SerpAPIClient:
    ...
    async def search_firm(self, firm_name: str) -> list[SerpResult]:
        ...
        if response is None:
            logger.warning("SerpAPI network error for '%s': %s", firm_name, last_exc)
            raise SerpAPIError(f"SerpAPI network error: {last_exc}") from last_exc
        ...
        if response.status_code != 200:
            ...
            raise SerpAPIError(f"SerpAPI returned {response.status_code}")
                ...
                if domain:
                    ...
                    results.append(
                        SerpResult(
                            ...
                            title=str(title) if isinstance(title, str) else "",
                            is_high_confidence=True,
                            snippet=(str(ab_text) if isinstance(ab_text, str) else ""),
                        )
                    )
