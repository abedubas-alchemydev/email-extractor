...
class AuthUser(Base):
    ...
    role: Mapped[str] = mapped_column(String(32), default="viewer", server_default=text("'viewer'"), nullable=False)
    # Auto-approve: new signups are active immediately (no admin gate in the
    # standalone). The parent leaves this defaultless and stamps it via a hook.
    status: Mapped[str] = mapped_column(
        String(16), default="active", server_default=text("'active'"), nullable=False, index=True
    )
    feature_permissions: Mapped[list[str]] = mapped_column(
        JSONB,
        default=list,
        # Single-feature standalone: every new user can use the extractor out of
        # the box. (The parent defaults to [] and grants features per-user.)
        server_default=text("""'["email_extractor"]'::jsonb"""),
        nullable=False,
    )
...
class Account(Base):
    ...
    email_address: Mapped[str | None] = mapped_column(String(320), nullable=True, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        server_default=func.now(),
